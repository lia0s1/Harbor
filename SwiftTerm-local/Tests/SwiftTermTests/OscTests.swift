...
final class SwiftTermOsc {
    ...
    @Test func testOscClipboardIsDisabledByDefaultAndCanBeEnabled() {
        let disabledDelegate = ClipboardDelegate()
        let disabled = Terminal(delegate: disabledDelegate, options: TerminalOptions())
        disabled.feed(text: "\u{1b}]52;c;aGVsbG8=\u{07}")
        #expect(disabledDelegate.copied.isEmpty)

        let enabledDelegate = ClipboardDelegate()
        let enabled = Terminal(
            delegate: enabledDelegate,
            options: TerminalOptions(allowOsc52ClipboardAccess: true)
        )
        enabled.feed(text: "\u{1b}]52;c;aGVsbG8=\u{07}")
        #expect(enabledDelegate.copied == [Data("hello".utf8)])
    }

    @Test func testOversizedOscIsDiscardedAcrossChunksAndParserRecovers() {
        let delegate = TitleDelegate()
        let terminal = Terminal(delegate: delegate, options: TerminalOptions())
        let half = EscapeSequenceParser.maxStringSequenceBytes / 2

        terminal.feed(byteArray: [0x1b, 0x5d, 0x30, 0x3b])
        terminal.feed(byteArray: [UInt8](repeating: 0x61, count: half))
        terminal.feed(byteArray: [UInt8](repeating: 0x61, count: half + 1))
        terminal.feed(byteArray: [0x07])

        #expect(delegate.titles.isEmpty)
        #expect(terminal.parser._osc.isEmpty)
        #expect(terminal.parser.currentState == .ground)

        terminal.feed(text: "\u{1b}]0;safe\u{07}")
        #expect(delegate.titles == ["safe"])
    }
