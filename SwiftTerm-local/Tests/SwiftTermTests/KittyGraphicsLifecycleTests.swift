...
final class KittyGraphicsLifecycleTests {
    ...
    @Test func testChunkedPayloadIsRejectedBeforePendingBufferCanExceedLimit() {
        let h = makeHeadlessTerminal()
        let t = h.terminal!
        var first = Array("a=t,f=32,s=1,v=1,t=d,q=1,m=1;".utf8)
        first.append(contentsOf: [UInt8](repeating: 0x41, count: Terminal.kittyMaxPendingBase64Bytes - 2))
        t.handleKittyGraphics(first[...])
        #expect(t.kittyGraphicsState.pending?.base64Payload.count == Terminal.kittyMaxPendingBase64Bytes - 2)

        let overflow = Array("m=1;AAAA".utf8)
        t.handleKittyGraphics(overflow[...])

        #expect(t.kittyGraphicsState.pending == nil)
        #expect(t.kittyGraphicsState.imagesById.isEmpty)
    }

    @Test func testKittyMemoryDefaultsAreBoundedForDesktopUse() {
        #expect(Terminal.kittyMaxImageBytes == 64 * 1024 * 1024)
        #expect(Terminal.kittyMaxPendingBase64Bytes == 8 * 1024 * 1024)
        #expect(TerminalOptions.default.kittyImageCacheLimitBytes == 128 * 1024 * 1024)
        #expect(Terminal.kittyMaxImageCacheBytes == 512 * 1024 * 1024)
    }
}
#endif
