...
final class ParserTests {
    ...
    @Test func testOversizedApcIsDiscardedAcrossChunksAndParserRecovers() {
        let parser = EscapeSequenceParser()
        var messages: [String] = []
        parser.apcHandlerFallback = { command, data in
            messages.append(String(bytes: [command] + Array(data), encoding: .utf8)!)
        }
        let half = EscapeSequenceParser.maxStringSequenceBytes / 2

        let prefix: [UInt8] = [0x1b, 0x5f, 0x58]
        parser.parse(data: prefix[...])
        let first = [UInt8](repeating: 0x61, count: half)
        parser.parse(data: first[...])
        let second = [UInt8](repeating: 0x61, count: half)
        parser.parse(data: second[...])
        let terminator: [UInt8] = [0x1b, 0x5c]
        parser.parse(data: terminator[...])

        #expect(messages.isEmpty)
        #expect(parser._apc.isEmpty)
        #expect(parser.currentState == .ground)

        let valid = Array("\u{1b}_Xok\u{1b}\\".utf8)
        parser.parse(data: valid[...])
        #expect(messages == ["Xok"])
    }

    @Test func testExcessiveParametersAndDigitsAreIgnored() {
        let parser = EscapeSequenceParser()
        var dispatchCount = 0
        parser.csiHandlerFallback = { _, _, _ in dispatchCount += 1 }

        let prefix: [UInt8] = [0x1b, 0x5b]
        parser.parse(data: prefix[...])
        let parameters = Array(String(repeating: "1;", count: EscapeSequenceParser.maxParameterCount + 1).utf8)
        parser.parse(data: parameters[...])
        let final: [UInt8] = [0x6d]
        parser.parse(data: final[...])
        #expect(dispatchCount == 0)
        #expect(parser._pars.count <= EscapeSequenceParser.maxParameterCount)

        let oversizedNumber = Array("\u{1b}[999999999999999999999999999999m".utf8)
        parser.parse(data: oversizedNumber[...])
        #expect(dispatchCount == 0)

        let valid = Array("\u{1b}[1m".utf8)
        parser.parse(data: valid[...])
        #expect(dispatchCount == 1)
    }

    @Test func testExcessiveCollectBytesAreIgnoredAcrossChunks() {
        let parser = EscapeSequenceParser()
        var dispatchCount = 0
        parser.escHandlerFallback = { _, _ in dispatchCount += 1 }

        let prefix: [UInt8] = [0x1b]
        parser.parse(data: prefix[...])
        let first = [UInt8](repeating: 0x20, count: EscapeSequenceParser.maxCollectBytes)
        parser.parse(data: first[...])
        let overflowAndFinal: [UInt8] = [0x20, 0x42]
        parser.parse(data: overflowAndFinal[...])

        #expect(dispatchCount == 0)
        #expect(parser._collect.count <= EscapeSequenceParser.maxCollectBytes)

        let valid: [UInt8] = [0x1b, 0x37]
        parser.parse(data: valid[...])
        #expect(dispatchCount == 1)
    }
}
