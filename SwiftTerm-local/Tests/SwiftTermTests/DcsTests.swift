...
final class DcsTests {
    ...
    @Test func testSixelRejectsOverflowingNumbersAndDimensions() {
        let h = HeadlessTerminal(queue: SwiftTermTests.queue) { _ in }
        let t = h.terminal!

        t.feed(text: "\(esc)Pq#0!999999999999999999999999999999~\(esc)\\")
        t.feed(text: "\(esc)Pq#0!\(SixelDcsHandler.maxDimension + 1)~\(esc)\\")
        let excessiveRows = String(repeating: "-", count: SixelDcsHandler.maxDimension / 6 + 2)
        t.feed(text: "\(esc)Pq#0\(excessiveRows)~\(esc)\\")

        #expect(h.images.isEmpty)
    }

    @Test func testSixelInputBufferHasHardLimitAcrossChunks() {
        let h = HeadlessTerminal(queue: SwiftTermTests.queue) { _ in }
        let handler = SixelDcsHandler(terminal: h.terminal)
        handler.hook(collect: [], parameters: [], flag: 0x71)

        let full = [UInt8](repeating: 0x7e, count: SixelDcsHandler.maxDataBytes)
        handler.put(data: full[...])
        let overflow: [UInt8] = [0x7e]
        handler.put(data: overflow[...])
        handler.unhook()
