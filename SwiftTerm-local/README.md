...
Termcast, benchmarks, and documentation plugins are excluded from the default
package graph so applications embedding the SwiftTerm library do not resolve
development-only remote dependencies. Opt in for development-tool commands in
the current shell:

```bash
export SWIFTTERM_INCLUDE_DEVELOPMENT_TARGETS=1
```
