...
public class EscapeSequenceParser {
    ...
    static let maxStringSequenceBytes = 1024 * 1024
    static let maxParameterCount = 64
    static let maxCollectBytes = 16
    static let maxParameterValue = Int(Int32.max)
    ...
    var _oscDiscarded: Bool
    var _apcDiscarded: Bool
    var _sequenceDiscarded: Bool
    ...
    func parse (data: ArraySlice<UInt8>)
    {
        ...
        var oscDiscarded = self._oscDiscarded
        var apcDiscarded = self._apcDiscarded
        var sequenceDiscarded = self._sequenceDiscarded
        ...
        while i < end {
            ...
            if currentState == .csiParam && (code > 0x2f && code < 0x3a) {
                if !sequenceDiscarded,
                   let newValue = EscapeSequenceParser.appendingDigit(to: pars[pars.count - 1], digit: Int(code) - 48) {
                    pars[pars.count - 1] = newValue
                } else {
            ...
            case .csiDispatch:
                ...
                sequenceDiscarded = false
            case .param:
                if !sequenceDiscarded {
                    if code == 0x3b || code == 0x3a {
                        if pars.count < EscapeSequenceParser.maxParameterCount {
                            parsTxt.append(code)
                            pars.append (0)
                        } else {
                            sequenceDiscarded = true
                        }
                    } else if let newValue = EscapeSequenceParser.appendingDigit(to: pars[pars.count - 1], digit: Int(code) - 48) {
                        pars[pars.count - 1] = newValue
                    } else {
                        sequenceDiscarded = true
                    }
            ...
            case .collect:
                if !sequenceDiscarded {
                    if collect.count < EscapeSequenceParser.maxCollectBytes {
                        collect.append (code)
                    } else {
                        sequenceDiscarded = true
                    }
            ...
            case .clear:
                ...
                printStateReset()
            case .dcsHook:
                if !sequenceDiscarded, let handler = dispatchDcs(collect: collect, code: code, pars: pars) {
                    dcsHandler = handler
                ...
                while j < end {
                    ...
                    if c == ControlCodes.BEL || c == ControlCodes.CAN || c == ControlCodes.ESC {
                        ...
                        if currentState == .apcString {
                            if !apcDiscarded {
                                if apc.count < EscapeSequenceParser.maxStringSequenceBytes {
                                    apc.append(c)
                                } else {
                                    apc = []
                                    apcDiscarded = true
                            ...
                            if !oscDiscarded {
                                if osc.count < EscapeSequenceParser.maxStringSequenceBytes {
                                    osc.append(c)
                                } else {
                                    osc = []
                                    oscDiscarded = true
            ...
            case .oscEnd:
                if currentState == .apcString {
                    if !apcDiscarded && apc.count != 0 && code != ControlCodes.CAN && code != ControlCodes.SUB {
                    ...
                    if !oscDiscarded && osc.count != 0 && code != ControlCodes.CAN && code != ControlCodes.SUB {
                        var oscCode: Int
        ...
        _pars = pars
        _parsTxt = parsTxt
        _oscDiscarded = oscDiscarded
        _apcDiscarded = apcDiscarded
        _sequenceDiscarded = sequenceDiscarded
    ...
    static func parseInt (_ str: ArraySlice<UInt8>) -> Int
    {
        ...
        for x in str {
            ...
            guard let newValue = appendingDigit(to: result, digit: Int(x - 48)) else {
                return 0
            }
            result = newValue
        }
    ...
    private static func appendingDigit(to value: Int, digit: Int) -> Int? {
        let (multiplied, multiplyOverflow) = value.multipliedReportingOverflow(by: 10)
        guard !multiplyOverflow else {
            return nil
        }
        let (result, addOverflow) = multiplied.addingReportingOverflow(digit)
        guard !addOverflow, result <= maxParameterValue else {
            return nil
        }
        return result
    }
