...
extension Terminal {
    ...
    static let kittyMaxPendingBase64Bytes = 8 * 1024 * 1024
    private static let kittyMaxImageDimension = 8192
    static let kittyMaxImageCacheBytes = 512 * 1024 * 1024
    ...
    func handleKittyGraphics(_ data: ArraySlice<UInt8>) {
        ...
        if control.more == 1 {
            if var pending = kittyGraphicsState.pending {
                guard pending.base64Payload.count <= Terminal.kittyMaxPendingBase64Bytes,
                      payload.count <= Terminal.kittyMaxPendingBase64Bytes - pending.base64Payload.count else {
                    kittyGraphicsState.pending = nil
                    sendKittyError(control: pending.control, message: "E2BIG: payload too large")
                    return
                }
                pending.base64Payload.append(contentsOf: payload)
                kittyGraphicsState.pending = pending
            } else {
                guard payload.count <= Terminal.kittyMaxPendingBase64Bytes else {
                    sendKittyError(control: control, message: "E2BIG: payload too large")
                    return
                }
    ...
    private func validateKittyDimensions(width: Int, height: Int) -> Bool {
        ...
        guard width <= Terminal.kittyMaxImageDimension, height <= Terminal.kittyMaxImageDimension else {
            return false
        }
        let pixelCount = Int64(width) * Int64(height)
        return pixelCount <= Int64(Terminal.kittyMaxImageBytes / 4)
    }
    ...
    private func validateKittyRawDimensions(width: Int, height: Int, bytesPerPixel: Int) -> Bool {
        ...
        let pixelCount = Int64(width) * Int64(height)
        let storedBytesPerPixel = max(bytesPerPixel, 4)
        let limit = Int64(Terminal.kittyMaxImageBytes) / Int64(storedBytesPerPixel)
