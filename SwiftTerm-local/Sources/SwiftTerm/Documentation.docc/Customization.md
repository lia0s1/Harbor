...
| `allowOsc52ClipboardAccess` | `false` | Allow terminal output to request clipboard writes |
| `kittyImageCacheLimitBytes` | 128 MB | Memory limit for Kitty image cache |
| `ansi256PaletteStrategy` | `.base16Lab` | 256-color palette generation strategy |

OSC 52 clipboard writes are disabled by default because escape sequences from a
remote process are untrusted. Enable `allowOsc52ClipboardAccess` only when the
embedding application has established an appropriate trust or consent boundary;
normal user-initiated copy operations are unaffected.
