...
class TermcastRecorder {
    ...
    private static let defaultShell = "/bin/bash"
    private static let trustedSystemShells: Set<String> = {
        let fallback = ["/bin/bash", "/usr/bin/bash", "/bin/zsh", "/usr/bin/zsh"]
    ...
    private static func sanitizedShell(_ shell: String?) -> String {
        guard let shell else { return defaultShell }
        let trimmed = shell.trimmingCharacters(in: .whitespacesAndNewlines)
        guard trustedSystemShells.contains(trimmed) else { return defaultShell }
        return trimmed
    }
    ...
    func record(to filePath: String, command: String?, timeout: Double? = nil) throws {
        ...
        let executable = Self.sanitizedShell(ProcessInfo.processInfo.environment["SHELL"])
        let args: [String]
    ...
    private func writeHeader(command: String?) throws {
        ...
        let header = AsciicastHeader(
            ...
            title: nil,
            env: [
                "SHELL": Self.sanitizedShell(env["SHELL"]),
