...
- 最终修复源码：Release 通用 archive 构建退出码 0；沙箱外 `codesign --verify --deep --strict` 通过，主二进制架构为 `x86_64 arm64`，Team ID 为 `XXXXXXXXXX` 且 hardened runtime 已启用。
...
- 安装版标识为 `dev.zero.Harbor`，Team ID `XXXXXXXXXX`，hardened runtime 已启用，主二进制架构为 `x86_64 arm64`，版本为 `1.0.0 (1)`。
...
- 安装版签名团队为 `XXXXXXXXXX`，带 hardened runtime，且二进制包含 `x86_64 arm64`。
...
- `security find-identity -v -p codesigning` 确认 `Developer ID Application: Harbor Developer (XXXXXXXXXX)` 有效。
